...
| Practice: plan (`/practice`) | Stored `shots.sg`, last 6 rounds | A | Needs 3 rounds; a focus range needs 3 shots; areas under 10 shots say "treat it as a hint" | `plan.test.ts` | — |
| Practice: drill library | `practice/data/drills.json` | C | 5 drills: no drill yet for off the tee, short game, recovery, fairway bunkers, putts under 4 ft, 8–30 ft or over 50 ft, approach over 250y (the page says so for the range); pass marks hand-set, not calibrated to handicap (the page says so) | `drills.test.ts` (shape, scored, random) | #61 |
