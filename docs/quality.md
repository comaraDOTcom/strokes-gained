...
| Practice: drill library | `practice/data/drills.json` | C | 5 drills: no drill yet for off the tee, short game, recovery, fairway bunkers, 10–20 ft putts; pass marks hand-set, not calibrated to handicap (the page says so) | `drills.test.ts` (shape, scored, random) | #61 |
