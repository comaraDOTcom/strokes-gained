...
| **Played: course directory** | `directory/data/ireland.json` from OpenStreetMap | **B → A** | 409 courses; **67% no hole count**; Killarney, Ceann Sibéal, Seapoint, Glasson, Athlone, County Louth (as "Baltray"), Carton House (by course names only) missing or misnamed; 2 duplicate clubs; 4 non-courses; county counts thin (Kerry 13, Louth 4, Limerick 6, Westmeath 4) | `data-quality.test.ts` (ratchet + known lists) | #38 |
