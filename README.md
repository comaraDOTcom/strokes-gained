...
**Better Than Most** (BTM) is strokes gained for competitive club golfers and their coaches: log shots hole-by-hole, get real strokes-gained
...
[`docs/brand/landing.html`](./docs/brand/landing.html). The name lives in `src/lib/brand.ts`, and
`src/lib/brand.test.ts` fails if "Strokes Gained" reappears as a product name or "leak" as copy.
The repo is still called `strokes-gained` (renaming it can wait).

The app mark (`src/app/logo.tsx`, `src/app/icon.svg`, `src/app/apple-icon.png`, `public/icon-*.png`
— keep them in sync with `scripts/make-pwa-icons.sh`) is "BTM" in IBM Plex Mono SemiBold, gold on
a forest-green disc, drawn as outlines so it renders without the font. The illustrated backdrop comes in four seasons —
